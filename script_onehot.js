...
async function clearIndexedDB() {
	...
	deleteRequest.onblocked = () => {
		console.warn(
			"Операция удаления заблокирована. Повторная попытка через 1 секунду..."
		);
		setTimeout(() => {
...
<div class="column-row">
    ...
    <select id="data_type_${columnCount}" name="data_type_column_name_${columnCount}" required title="Выберите тип данных">
        ...
        <option value="date" ${
					dataType === "date" ? "selected" : ""
				}>date</option>
        <option value="time" ${
					dataType === "time" ? "selected" : ""
				}>time</option>
        <option value="float_range" ${
					dataType === "float_range" ? "selected" : ""
				}>float_range</option>
        <option value="integer_range" ${
					dataType === "integer_range" ? "selected" : ""
				}>integer_range</option>
        <option value="range_negative" ${
					dataType === "range_negative" ? "selected" : ""
				}>range_negative</option>
        <option value="choice" ${
					dataType === "choice" ? "selected" : ""
				}>choice</option>
        <option value="boolean" ${
					dataType === "boolean" ? "selected" : ""
				}>boolean</option>
        <option value="name" ${
					dataType === "name" ? "selected" : ""
				}>name</option>
        <option value="phone" ${
					dataType === "phone" ? "selected" : ""
				}>phone</option>
        <option value="email" ${
					dataType === "email" ? "selected" : ""
				}>email</option>
        <option value="random" ${
					dataType === "random" ? "selected" : ""
				}>random</option>
        ${
					!isCSVLoaded
						? `<option value="fromloadcsv" ${
								dataType === "fromloadcsv" ? "selected" : ""
						  }>fromloadcsv</option>`
						: ""
				}
    </select>
...
`;
	...
	document
		.getElementById(`data_type_${columnCount}`)
		.addEventListener("change", function () {
			const dataType = this.value;
			const dataRangeInput = document.getElementById(
				`data_range_${columnCount}`
			);
...
function removeColumn(columnIndex) {
	const column = document.getElementById(`column_name_${columnIndex}`)
...
function renumberColumns() {
	...
	columns.forEach((column, index) => {
		...
		const dataTypeSelect = column.querySelector(
			'select[name^="data_type_column_name_"]'
		);
		const dataRangeTextarea = column.querySelector(
			'textarea[name^="data_range_column_name_"]'
		);
		...
		if (columnNameInput && dataTypeSelect && dataRangeTextarea && rowNumber) {
			...
			console.error(
				`Один или несколько элементов не найдены для колонки ${index + 1}`
			);
		}
	});
}
...
function updateDataRangeVisibility(columnNumber) {
	...
	const dataRangeTextarea = document.getElementById(
		`data_range_${columnNumber}`
	);

	if (dataTypeSelect && dataRangeTextarea) {
		dataTypeSelect.addEventListener("change", function () {
			if (
				dataTypeSelect.value === "float_range" ||
				dataTypeSelect.value === "date" ||
				dataTypeSelect.value === "integer_range" ||
				dataTypeSelect.value === "range_negative" ||
				dataTypeSelect.value === "choice" ||
				dataTypeSelect.value === "time"
			) {
...
async function generateCSV() {
	if (uploadedData) {
		...
		if (indexType === "auto") {
			...
			const customIndexRange =
				document.getElementById("customIndexRange").value;
		...
		const csvContent = [columns.map(escapeCSV).join(",")]
			.concat(
				data[0].map((_, i) =>
					data.map((col) => escapeCSV(col[i] || "")).join(",")
				)
			)
			.join("\n");
...
function generateData(dataType, dataRange, numRows, columnName) {
	if (dataType === "date") {
		...
		if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
			alert(
				"Некорректный формат даты. Используйте формат ГГГГ.ММ.ДД-ГГГГ.ММ.ДД."
			);
			return [];
		}
		...
		return Array.from({ length: numRows }, () => {
			const randomDate = new Date(
				startDate.getTime() + Math.random() * delta * 24 * 60 * 60 * 1000
			);
		...
		const { maleFirstNames, femaleFirstNames, maleLastNames, femaleLastNames } =
			namesData;
		return Array.from({ length: numRows }, () => {
...
function generateRandomTime(startTime, endTime) {
	...
	const randomTotalSeconds =
		Math.floor(Math.random() * (endTotalSeconds - startTotalSeconds + 1)) +
		startTotalSeconds;
...
function validateInput(columnName, dataType, dataRange) {
	...
	if (
		dataType === "float_range" ||
		dataType === "integer_range" ||
		dataType === "range_negative" ||
		dataType === "choice" ||
		dataType === "time" ||
		dataType === "date"
	) {
		...
		if (dataType === "float_range" || dataType === "integer_range") {
			...
			if (!dateRegex.test(dataRange)) {
				alert(
					"Некорректный формат даты. Используйте формат ДД.ММ.ГГГГ-ДД.ММ.ГГГГ"
				);
				return false;
			}
			const [startDateStr, endDateStr] = dataRange.split("-");
			...
			if (!timeRegex.test(dataRange)) {
				alert(
					"Некорректный формат времени. Используйте формат ЧЧ:ММ:СС-ЧЧ:ММ:СС."
				);
				return false;
			}
			const [startTime, endTime] = dataRange.split("-");
...
function displayCSVData(csvContent) {
	...
	for (let i = 1; i <= rowsPerPage && i < csvLines.length; i++) {
	...
	if (paginationContainer) {
		...
		const currentPage =
			parseInt(paginationContainer.getAttribute("data-current-page"), 10) || 1;
...
function assignUniqueNumbers(columnData) {
	...
	const updatedColumnData = columnData.map(
		(value) => valueToNumber[value] || 0
	); // Используем 0 для неизвестных значений

	return { valueToNumber, updatedColumnData };

	return { valueToNumber, numberToValue, updatedColumnData };
	console.log("Словарь уникальных номеров:", valueToNumber);
	console.log("Обновленные данные столбца:", updatedColumnData);
}
...
async function analyzeColumnData(columnName) {
	...
	const uniqueValues = Object.keys(valueCounts).filter(
		(value) => valueCounts[value] === 1
	);
	const duplicateValues = Object.keys(valueCounts).filter(
		(value) => valueCounts[value] > 1
	);
	...
	let { valueToNumber, numberToValue, updatedColumnData } =
		assignUniqueNumbers(columnData);
 ...
 */
async function createAndTrainModel(inputShape, xTrain, yTrain, columnMetadata) {
	try {
		// Проверяем, является ли xTrain массивом
		if (!Array.isArray(xTrain) || !Array.isArray(yTrain)) {
			throw new Error("xTrain и yTrain должны быть массивами");
		}

		// Преобразуем данные в числовые значения
		const numericXTrain = xTrain.map((row) =>
			row.map((value, colIndex) => {
				const columnName = Object.keys(columnMetadata)[colIndex];
				const metadata = columnMetadata[columnName];
				if (metadata.dataType === "name" || metadata.dataType === "choice") {
					return metadata.uniqueNumbers[value] || 0; // Используем 0, если значение не найдено
				}
				return parseFloat(value) || 0; // Преобразуем в число, если это возможно
			})
		);

		const numericYTrain = yTrain.map(
			(row) => row.map((value) => parseFloat(value) || 0) // Преобразуем в число, если это возможно
		);

		// Убедитесь, что данные являются числовыми массивами
		const xTrainTensor = tf.tensor2d(numericXTrain);
		const yTrainTensor = tf.tensor2d(numericYTrain);
		...
		model.add(
			tf.layers.dense({ units: yTrainTensor.shape[1], activation: "linear" })
		);
...
async function analyzeWordFrequency(columnName) {
	...
	const columnData = data
		.map((row) => (row[columnIndex] !== undefined ? row[columnIndex] : ""))
		.join(" ")
		.toLowerCase();
...
function buildHeatmap(data, headers) {
	...
	const numericColumns = headers.filter((header) => {
		return data.every(
			(row) => !isNaN(parseFloat(row[headers.indexOf(header)]))
		);
	});
...
function calculateCorrelation(col1, col2) {
	...
	const numerator = col1
		.map((val, i) => (val - mean1) * (col2[i] - mean2))
		.reduce((sum, val) => sum + val, 0);
	const denominator1 = col1
		.map((val) => Math.pow(val - mean1, 2))
		.reduce((sum, val) => sum + val, 0);
	const denominator2 = col2
		.map((val) => Math.pow(val - mean2, 2))
		.reduce((sum, val) => sum + val, 0);
...
async function showAnalytics() {
	try {
		...
		let metricsHeaders = headersIndexedDB.filter(
			(header) => !isNaN(Number(parsedData.data[0][header]))
		);
		let dataDb = parsedData.data.map((row) =>
		...
		means = means.map((mean, index) =>
			count[index] > 0 ? mean / count[index] : 0
		);

		dataDb = dataDb.map((row) =>
			row.map((value, colIndex) => (value !== null ? value : means[colIndex]))
		);
		...
		const xTrain = normalizedData.slice(
			[0, 0],
			[splitIndex, normalizedData.shape[1]]
		);
		const yTrain = normalizedData.slice(
			[0, 0],
			[splitIndex, normalizedData.shape[1]]
		);
		const xTest = normalizedData.slice(
			[splitIndex, 0],
			[-1, normalizedData.shape[1]]
		);
		...
		meanPredictionsArray.forEach((value, index) => {
			...
			if (numberToValue && numberToValue[value] !== undefined) {
				let decodedValue = numberToValue[value];
			} else if (typeof value === "number") {
				let decodedValue = value.toFixed(2);
			} else {
				console.error(
					"Значение не является числом или не найдено в numberToValue."
				);
			}

			let decodedValue;
			if (numberToValue && numberToValue[value] !== undefined) {
				decodedValue = numberToValue[value];
			} else if (typeof value === "number") {
				decodedValue = value.toFixed(2);
			} else {
				console.error("Значение не является числом.");
			}

			decodedValue =
				numberToValue && numberToValue[value] !== undefined
					? numberToValue[value]
					: typeof value === "number"
					? value.toFixed(2)
					: value;
      ...
      `;
		...
		for (let i = 0; i < headers.length; i++) {
			...
			try {
				...
				wordFrequencyCell.innerHTML = JSON.stringify(
					wordFrequencyResult.wordFrequency,
					null,
					2
				);
				...
				uniqueNumbersCell.innerHTML = JSON.stringify(
					analysisResult.uniqueNumbers,
					null,
					2
				);
...
print(df)
    `;
	const jupyterLink = `https://jupyter.org/try?example=${encodeURIComponent(
		code
	)}`;
...
print(df)
    `;
	const colabLink = `https://colab.research.google.com/notebook#create=true&code=${encodeURIComponent(
		code
	)}`;
