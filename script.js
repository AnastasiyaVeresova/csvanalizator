...
Original file is located at
    https://colab.research.google.com/drive/1VWa_T9zUyyPV29NUCXnwfTEpAIgLdIAr
"""
...
df = pd.read_csv('sample_data/file.csv', encoding='utf-8')
print(df.head())
...
let csvContent = localStorage.getItem("csvData");
  if (!csvContent) {
    ...
    columnData.forEach(value => {
        console.log(`Обрабатываемое значение: ${value}`);
        if (value === null) {
            nullValues++;
            console.log(`nullValues увеличено: ${nullValues}`);
        } else if (value === undefined) {
            undefinedValues++;
            console.log(`undefinedValues увеличено: ${undefinedValues}`);
        } else if (value === '' || value === 'NaN') {
            nanValues++;
            console.log(`nanValues увеличено: ${nanValues}`);
        }

        if (value === '' || value === null || value === undefined || value === 'NaN') {
            missingValues++;
            console.log(`missingValues увеличено: ${missingValues}`);
        }
    });
...
async function showAnalytics() {
  ...
  meanPredictionsArray.forEach(value => {
      const td = document.createElement('td');
